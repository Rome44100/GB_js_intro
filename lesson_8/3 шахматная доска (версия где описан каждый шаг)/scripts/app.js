let app = {
    ...
    // Не очень получилось с иконками пешек: из awesome подгружаются почему только черные и не пнг
    // обычные коды хтмл почему то только белые показывает, не знаю :)
    insertPawns() {
        // 6. получаем все теги td из 7 линии игрового поля, туда мы будем вставлять белые пешки
        let whitePawnsRow = document.querySelectorAll("tr")[6].querySelectorAll("td");
        for (let i = 0; i < whitePawnsRow.length; i++) {
            ...
            whitePawnsRow[i].innerHTML = "&#9817";
        }

        // 6.2 получаем все теги td из 2 линии игрового поля, туда мы будем вставлять черные пешки
        let blackPawnsRow = document.querySelectorAll("tr")[1].querySelectorAll("td");
        for (let i = 0; i < blackPawnsRow.length; i++) {
            ...
            blackPawnsRow[i].innerHTML = "&#9823";
        }
    },
